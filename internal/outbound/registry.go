...
import (
	...
	"github.com/lihongjie0209/tenant-service/internal/grpcclient"
...
func NewRegistry(lc fx.Lifecycle, cfg config.Config, metrics *observability.Metrics) (*Registry, error) {
	...
	for name, upstream := range cfg.Outbound.GRPC {
		clientCfg := grpcclient.Config{Name: name, Target: upstream.Target, Timeout: upstream.Timeout, Retry: upstream.Retry, Breaker: upstream.Breaker, Metrics: metrics, TLS: grpcclient.TLSConfig{Enabled: upstream.TLS.Enabled, ServerName: upstream.TLS.ServerName, CAFile: upstream.TLS.CAFile, CertFile: upstream.TLS.CertFile, KeyFile: upstream.TLS.KeyFile, AllowInsecureToken: upstream.AllowInsecureCredentials}}
